---
title: "IISER Advanced Bioacoustics Workshop - BirdNET thresholding for detection data"
date: "February 2026"
output: 
  html_document:
    code_folding: hide
...
```{r thresholds}
  #generate a prediction range from the min and max confidence score
  prediction.range.conf=seq(0,1,.001) 
  #generate a prediction range from the min and max logit score
  ...
  cutoff90.c=(log(.90/(1-.90))-c.intercept)/c.beta; cutoff90.c
  cutoff95.c=(log(.95/(1-.95))-c.intercept)/c.beta; cutoff95.c
  cutoff99.c=(log(.99/(1-.99))-c.intercept)/c.beta; cutoff99.c
  
  #logit model
  l.intercept = logit.model$coefficients[1]
  l.beta = logit.model$coefficients[2]
  cutoff90.l=(log(.90/(1-.90))-l.intercept)/l.beta; cutoff90.l
  cutoff95.l=(log(.95/(1-.95))-l.intercept)/l.beta; cutoff95.l
  cutoff99.l=(log(.99/(1-.99))-l.intercept)/l.beta; cutoff99.l
...
To decide which threshold to use can depend on whether it is more important in your study to include as many true positives as possible (i.e., high recall) or to strictly ensure few false positives are included (i.e., high precision). Identifying how many true positives may be removed and how many false positives may be included under each threshold can be a useful way to determine which threshold you want to use.
...
#summary of FP, FN, TN, TP with 90% threshold
...
#summary of FP, FN, TN, TP with 95% threshold
...
#summary of FP, FN, TN, TP with 99% threshold
...
From this we can see that, for this validation dataset, all thresholds resulted in equal precision: no false positives were included. However, it is important to consider that when applied to the full dataset false positives will likely be present.

In terms of recall, there was greater variability. With a 99% threshold, 31% of true positives would be removed. That may not be a big deal if detections are common, but it is worth considering in your choice of threshold.

You may want to check how different thresholds affect the number of sites that have detections or not (i.e., naive occupancy). Ideally, the thresholds will produce the same number of sites where an animal was detected at least once. It can be a good check to see if, at a given threshold, a species is detected in sites where you can be reasonably confident it should NOT be (i.e., you may be including too many false positives).

It is an open area of research to determine exactly how to translate continuous detection scores into an occupancy framework. Occupancy can be very sensitive to false positives, but false negatives may be the bigger concern in other applications (e.g., when making environmental decisions that will result in land prioritization/protection).

## 5) Creating a detection history

Let us assume we choose a 90% threshold. We would then subset the full BirdNET output to include only detections that meet or exceed the threshold value and create a site-by-visit detection history. For the purpose of this tutorial we will pretend the validation dataset is the full output

```{r create detection history}

#subset the data by the selected threshold 
  #NOTE: this would be the full BirdNET output, not just the validation data
data.90 <- subset(data, logit >= cutoff90.l)

#extract the site name 
  #NOTE: this is unique to these file names, you will have to adjust for your own file names
